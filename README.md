...
- `/notices` 페이지: 아코디언 목록 + 신규 뱃지
- 푸터 링크로 접근 (공지사항 · 개발자에게)
...
- 404, 로딩 상태 페이지
- 아이콘: `react-icons/io` (`IoIosArrowForward`, `IoIosArrowBack`, `IoIosArrowDown`)
...
- `/notices` 공지사항 페이지 (아코디언 목록, 신규 뱃지)
- 푸터 공지사항·개발자에게 링크, 단일 행 패턴 + GitHub SVG 아이콘 + 동적 연도

#### Changed

- 비공개 방명록 인증: 관리자 PIN 전용 → 항목별 비밀번호 + 관리자 PIN(마스터)으로 변경
- 전체 텍스트 화살표(`→` `←` `▼`) → `react-icons/io` 아이콘으로 교체
- Notion 방명록 DB에 `Password` (rich_text) 속성 추가
- 방명록 페이지 상단 업데이트 공지 배너 추가
...
비공개 방명록에 **비밀번호 기능**이 추가되었습니다.

- 비공개 메시지 작성 시 열람 비밀번호를 직접 설정할 수 있습니다.
- 이전(2026년 4월 30일 이전)에 작성된 비공개 방명록 글은 비밀번호가 **0000**으로 초기화됩니다.
